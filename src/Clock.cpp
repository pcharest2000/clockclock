...
uint32_t Clock::getHourIndex(uint32_t i, uint32_t j) {

  return j * CLOCKWIDTH * 2 + i * 2 + 1;
}
uint32_t Clock::getMinuteIndex(uint32_t i, uint32_t j) {

  return j * CLOCKWIDTH * 2 + i * 2;
}
void Clock::setMinuteTarget(uint32_t i, uint32_t j, int32_t pos,
                            uint32_t startSpeed, uint32_t endSpeed,
                            int32_t stepsWidth) {
...
void Clock::setHourTarget(uint32_t i, uint32_t j, int32_t pos,
                          uint32_t startSpeed, uint32_t endSpeed,
                          int32_t stepsWidth) {
  if (i >= CLOCKWIDTH || j >= CLOCKHEIGTH) {
    return;
